...
def readScreentimeData(path: Path | str | None = None, min_days: int = 7) -> df:
    ...
    screentime = screentime[screentime['nt_y_earsapp__day_count'] >= min_days]
    ...
    screentime.rename(columns={'mean': 'total'}, inplace=True)
    return screentime


def readStaticVars(path: Path | str | None = None) -> df:

    if path is None:
        path = generalDirectory.joinpath('ab_g_stc.tsv')

    data = pd.read_csv(path, sep='\t')
    data = data[['participant_id', 'ab_g_stc__cohort_sex']]
    data.rename(columns={'ab_g_stc__cohort_sex': 'sex'}, inplace=True)
    return data


def readDynamicVars(path: Path | str | None = None, filter_session: bool = False) -> df:

    if path is None:
        path = generalDirectory.joinpath('ab_g_dyn.tsv')
    ...
    vars = {
        ...
        'ab_g_dyn__design_mr__serial': 'scanner_serial',
        'ab_g_dyn__design_mr__software': 'scanner_software'
    }

    data = data[list(vars.keys())]
...
def readGraphMetrics(path: Path | str | None = None) -> df:

    if path is None:
        path = dataDirectory.joinpath('graph_metrics.csv')
...
def __readOrganizedImagingData(
    ...
    :param mask: Mask to be applied to datasets (i.e., to return a specific session). 
                 Redundant for now, as the data is filtered when saved
...
def readGpnetAsegCorr(
        path: Path | str | None = None, 
        corrmat_key: str | None = 'corrmat',
        net_vec_key: str | None = 'net_vec',
        network_names_key: str | None = 'network_names',
        roi_names_key: str | None = 'roi_names'
    ) -> List[ndarray]:
...
def readAdjustedFcMatrices(
        path: Path | str | None = None,
        corrmat_key: str | None = 'corrmat',
        roi_vec_key: str | None = 'roi_vec',
        roi_names_key: str | None = 'roi_names',
        participant_id_key: str | None = 'participant_id'
    ) -> List[ndarray]:
    
    '''
    Pass None instead of key names to avoid including
    NOTE: I couldn't save the IDs in an HDF file so participant_id_key parameter doesn't function as intended but rather
    only as binary "Include"/"Don't include", which reads IDs from a predefined file
    '''

    if path is None:
        path = imagingDirectory.joinpath(r'organized\fc_adjusted.h5')
    data = []

    if corrmat_key is not None:
        keys, is_str, is_filterable = [corrmat_key], [False], [True]
        data.append(__readOrganizedImagingData(path, keys, is_str, is_filterable)[0])
    
    if (roi_vec_key is not None) or (roi_names_key is not None):
        for dataset in readGpAsegCorr(None, None, None, roi_vec_key, roi_names_key):
            data.append(dataset)

    if participant_id_key is not None:
        id_path = imagingDirectory.joinpath(r'organized\fc_adjusted_participant_id.npy')
        data.append(np.load(id_path, allow_pickle=True))
