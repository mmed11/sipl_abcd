...
def pairwise_network_global_efficiencies(
    ...
    G = None
    if inverse_weights:
        G = nx.from_numpy_array(extras.ndarray_reciprocal(adj_mat))
    else:
...
def create_graph(adj_mat: ndarray, nodes_to_remove: List | None = None) -> nx.Graph:
...
def compute_metrics(
    ...
    metrics.update(pairwise_network_global_efficiencies(
        abs_fc_matrix, # weighted no threshold adjacency matrix
        inverse_atlas, 
        inverse_weights=True,
        prefix='NoThresh'
    ))
    ...
    for network_name, network_nodes in inverse_atlas.items():
        ...
        unweighted_graph = create_graph(adj_matrix, nodes_to_remove)
        weighted_graph = create_graph(wgt_matrix, nodes_to_remove)
        nothresh_graph = create_graph(fc_matrix_network, nodes_to_remove)
        inverse_weighted_graph = create_graph(extras.ndarray_reciprocal(wgt_matrix), nodes_to_remove)
        inverse_nothresh_graph = create_graph(extras.ndarray_reciprocal(fc_matrix_network), nodes_to_remove)

        assert nothresh_graph.number_of_nodes() == network_size
        ...
        metrics[prefix + 'NoThresh_Efficiency'] = global_efficiency(inverse_nothresh_graph, weight='weight')    
        ...
        metrics[prefix + 'NoThresh_Clustering'] = nx.average_clustering(nothresh_graph, weight='weight')
        ...
        metrics[prefix + 'NoThresh_Modularity'] = nx.community.modularity(
            weighted_graph, nx.community.louvain_communities(nothresh_graph, weight='weight'), weight='weight'
        )

        # Substitution with a LCC for metrics requiring a connect graph
        ...
        inverse_weighted_lcc_graph = inverse_weighted_graph.subgraph(lcc_nodes).copy()
        inverse_nothresh_lcc_graph = inverse_nothresh_graph.subgraph(lcc_nodes).copy()
        ...
        metrics[prefix + 'Weighted_AvgShortestPath'] = nx.average_shortest_path_length(inverse_weighted_lcc_graph, weight='weight')
        metrics[prefix + 'NoThresh_AvgShortestPath'] = nx.average_shortest_path_length(inverse_nothresh_lcc_graph, weight='weight', method='floyd-warshall-numpy')
...
if __name__ == '__main__':
    ...
    inverse_atlas[''] = list(range(1, 367)) # Whole brain
